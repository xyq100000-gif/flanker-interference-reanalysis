...
2. **Behavioral data only**
   The experiment includes reaction-time and error summaries only. It does not support direct claims about neural mechanisms.

3. **No trial-level stream**
   The raw file preserves repeated exports within condition, but not trial-by-trial observations. The block-level reanalysis therefore improves the structure relative to the original summary, but it is still not a full trial-level model.

4. **Partial missingness in errors**
